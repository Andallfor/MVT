...
public class Calc
{
    ...
    public static double topo(Timeline sat, Timeline center, geographic geo, double alt, double jd)
    {
        double Deg2Rad = Math.PI / 180.0;
        position rSite = geo.toCartesianWGS(alt);
        position satSwitch = sat.find(new Time(jd)) + center.find(new Time(jd));
        position rSat = position.ECI2ECEF(new position(satSwitch.x, satSwitch.z, satSwitch.y), jd) - rSite;
