...
public class ElevationCheck
{
    ...
    public static List<double[]> elevationTimes(Timeline sat, Timeline center, geographic geo, double alt, double minEl, double duration, double jd)
    {
        double startEpoch = 0;
        double step = 60 * 2.5;
        const double muE = 398600.44;

        return Contacts.runContacts(sat, center, geo, alt, minEl, step, duration, jd);
    }
}
